...
const VisionSection = () => {
    ...
    return (
        <section ref={containerRef} className="w-full bg-transparent py-12 md:py-24 px-4 md:px-8 flex flex-col items-center">
            ...
            <div className="w-full max-w-7xl mx-auto min-h-[600px] flex flex-col md:flex-row gap-4">
                {cards.map((card) => (
                    <div
                        key={card.id}
                        ...
                        className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] group 
                            ${activeId === card.id ? "grow-[4] min-h-[400px] md:min-h-0" : "grow-[1] min-h-[80px] md:min-h-0"}
                        `}
                    ...
                    >
                        ...
                        <div className={`absolute inset-0 flex flex-col p-6 md:p-8 rounded-2xl transition-opacity duration-500 bg-[#0a0a0a3a] border border-[#0080FF52] ${activeId === card.id ? "opacity-100 delay-200 z-10" : "opacity-0 pointer-events-none z-0"}`}>
                            <div className="absolute top-0 left-0 bg-[#0080FF6B] text-white px-4 md:px-6 py-2 md:py-4 rounded-br-2xl text-[9px] md:text-[10px] font-bold tracking-widest uppercase flex items-center gap-2 z-20" style={{ backdropFilter: "blur(20px)" }}>
                        ...
                        {/* Collapsed State */}
                        <div className={`absolute rounded-2xl inset-0 flex md:flex-col transition-all duration-300 ${activeId === card.id ? "opacity-0 pointer-events-none delay-0" : "opacity-100 delay-100"}`}>
                            <div className={`h-full md:h-[18%] w-[60px] md:w-full p-4 flex flex-col gap-2 bg-[#0080FF52]`}>
                                <span className="w-1.5 h-1.5 bg-white block" />
                                <span className="hidden md:block text-white font-sans text-[12px] font-bold tracking-widest uppercase leading-loose w-max">
                                    {card.label}
                                </span>
                            </div>

                            <div className="grow relative overflow-hidden bg-[#0A0A0AA3] flex items-center justify-center">
                                <div className="md:-rotate-90 whitespace-nowrap origin-center transform md:translate-y-10 px-4">
                                    <h3 className="font-serif text-lg md:text-2xl text-white mb-0 md:mb-1">{card.title}</h3>
                                    <p className="hidden md:block font-serif text-white text-base md:text-lg">{card.subtitle}</p>
                                </div>
                            </div>
                        </div>
                    </div>
